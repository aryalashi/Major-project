...
COL_HEADERS = [
    ...
    "PORT", "PACKETS", "CATEGORY", "ALGORITHM"
]
...
class NIDSMainWindow(QMainWindow):
    ...
    def _build_search_bar(self):
        ...
        self.search_box.setPlaceholderText(
            "Filter by IP address, port, protocol, rule name, severity, category, algorithm..."
        )
        ...
        self.filter_col.addItems([
            ...
            "TYPE", "RULE NAME", "SEVERITY", "PORT", "CATEGORY", "ALGORITHM"
        ])
    ...
    _COL_MAP = {
        ...
        "SEVERITY": COL_SEV, "PORT": COL_PORT,
        "CATEGORY": COL_CAT, "ALGORITHM": COL_LAYER,
    }

    def _on_tab_changed(self, idx):
        hints = [
            "Filter by IP, port, rule, severity, category, algorithm...",
    ...
    @staticmethod
    def _is_payload_signature_alert(alert: dict) -> bool:
        """Return True only for payload/signature attack alerts."""
        atype = str(alert.get("type", "")).lower()
        rule = str(alert.get("rule_name", "")).lower()
        return (
            atype == "payload attack"
            or "signature" in atype
            or "signature" in rule
        )
    ...
    def _on_alert(self, alert: dict):
        ...
        if isinstance(category, list):
            category = ", ".join(str(x) for x in category) if category else "—"
        elif not category:
            category = "—"
        ...
        if atype == "Payload Attack":
            layer = "PAYLOAD SIGNATURE"
    ...
    def _on_packet(self, pkt: dict):
        payload_label = pkt.get("payload_label", "NO_PAYLOAD")
        if payload_label == "NO_PAYLOAD":
            return
        ...
        payload_category = pkt.get("payload_category", "No Payload")
        self.packet_log.appendPlainText(
        f"[{ts}]  {pkt.get('protocol','?'):<5}  "
        f"{pkt.get('src','?'):<18}  ->  "
        f"{pkt.get('dst','?'):<18}{p_s:<7}  "
        f"flags=[{flags_str:<4}]  "
        f"payload={payload_label:<8}  cat={payload_category}"
        )
