...
RULES = {
    ...
    "syn":        ("TCP SYN Flood",         ["TCP SYN FLOOD", "SYN FLOOD"],               120,  "TCP SYN flood to port 80"),
    "udp":        ("UDP Flood",             ["UDP FLOOD"],                                 120,  "UDP flood to random ports"),
    "icmp":       ("ICMP Flood",            ["ICMP FLOOD", "ICMP"],                        200,  "ICMP echo flood"),
    "ack":        ("TCP ACK Flood",         ["TCP ACK FLOOD", "ACK FLOOD"],                120,  "TCP ACK flood"),
    "rst":        ("TCP RST Flood",         ["TCP RST FLOOD", "RST FLOOD"],                120,  "TCP RST flood"),
    "scan":       ("Port Scan Detection",   ["PORT SCAN", "PORTSCAN", "PORT SCAN DETECTION"], 10, "TCP SYN port scan (variety)"),
    "fin_scan":   ("TCP FIN Scan",          ["FIN SCAN", "TCP FIN SCAN"],                  10,   "TCP FIN port scan (variety)"),
    "null":       ("TCP NULL Scan",         ["NULL SCAN", "TCP NULL SCAN"],                10,   "TCP NULL scan (flags=0)"),
    "xmas":       ("TCP XMAS Scan",         ["XMAS SCAN", "TCP XMAS SCAN"],               10,   "TCP XMAS scan (FIN+PSH+URG)"),
    "udpscan":    ("UDP Port Scan",         ["UDP PORT SCAN", "UDP SCAN"],                 10,   "UDP port scan (variety)"),
    "heartbleed": ("Heartbleed Attack",     ["HEARTBLEED"],                                20,   "Heartbleed (SYN to port 443)"),
    "ftp":        ("FTP Brute Force",       ["FTP BRUTE", "FTP BRUTE FORCE", "FTP"],       20,   "FTP brute force (SYN to port 21)"),
    "ssh":        ("SSH Brute Force",       ["SSH BRUTE", "SSH BRUTE FORCE", "SSH"],       20,   "SSH brute force (SYN to port 22)"),
    "telnet":     ("Telnet Brute Force",    ["TELNET BRUTE", "TELNET"],                    20,   "Telnet brute force (port 23)"),
    "rdp":        ("RDP Brute Force",       ["RDP BRUTE", "RDP"],                          20,   "RDP brute force (port 3389)"),
    "http":       ("HTTP Flood",            ["HTTP FLOOD", "HTTP SYN"],                   100,   "HTTP SYN flood (port 80)"),
    "https":      ("HTTPS Flood",           ["HTTPS FLOOD", "HTTPS SYN"],                 100,   "HTTPS SYN flood (port 443)"),
    "smtp":       ("SMTP Flood",            ["SMTP FLOOD", "SMTP"],                        30,   "SMTP flood (port 25)"),
    "dns":        ("DNS Amplification",     ["DNS AMPLIFICATION", "DNS"],                  50,   "DNS flood (UDP port 53)"),
    "mysql":      ("MySQL Brute Force",     ["MYSQL BRUTE", "MYSQL"],                      20,   "MySQL brute force (port 3306)"),
    "smb":        ("SMB Flood",             ["SMB FLOOD", "SMB"],                          30,   "SMB flood (port 445)"),
    "netbios":    ("NetBIOS Flood",         ["NETBIOS FLOOD", "NETBIOS"],                  40,   "NetBIOS flood (UDP 137)"),
    "ldap":       ("LDAP Flood",            ["LDAP FLOOD", "LDAP"],                        30,   "LDAP flood (port 389)"),
    "mssql":      ("MSSQL Brute Force",     ["MSSQL BRUTE", "MSSQL"],                      20,   "MSSQL brute force (port 1433)"),
    "postgres":   ("PostgreSQL Brute Force",["POSTGRESQL BRUTE", "POSTGRESQL", "POSTGRES"],20,   "PostgreSQL brute force (port 5432)"),
...
def attack_icmp(target, duration, rate, stop_evt):
    """ICMP Flood — echo request storm"""
    # Also launch Windows ping as backup for non-Scapy
    ping_proc = None
    try:
        ping_proc = subprocess.Popen(
            ["ping", "-n", str(duration * 200), "-l", "1400", "-w", "1", target],
...
def main():
    ...
    parser.add_argument(
        ...
        help=(
            ...
            "  Single:  syn, udp, icmp, ack, rst, scan, fin_scan, null, xmas,\n"
            "           udpscan, heartbleed, ftp, ssh, telnet, rdp, http, https,\n"
            "           smtp, dns, mysql, smb, netbios, ldap, mssql, postgres,\n"
            "           synack, fin_flood, psh_ack, winrm, elastic, ddos, normal\n"
