...
class _FlushFileHandler(logging.FileHandler):
    """File handler that flushes EVERY record immediately to disk for zero-delay logging."""
    def emit(self, record):
        try:
            super().emit(record)
            self.flush()  # Flush Python buffer
            # Force OS-level write to disk (critical for immediate visibility)
    ...
    def __init__(self, *args, **kwargs):
        ...
        # Disable buffering at Python level
        if hasattr(self.stream, 'reconfigure'):
            try:
                self.stream.reconfigure(line_buffering=False, write_through=True)
            except Exception:
                pass
...
class CaptureController:
    ...
    def __init__(self, detection_engine, alert_engine,
                 evidence_logger, auto_scanner, gui_queue,
                 payload_detector=None, hotspot_monitor=None):
        self.detection_engine = detection_engine  # also used by LiveTestTab
        self.alert_engine     = alert_engine
        self.evidence_logger  = evidence_logger
        self.auto_scanner     = auto_scanner
        self.payload_detector = payload_detector
        ...
        self._payload_attack_log = "logs/payload_attacks.jsonl"
        self._payload_normal_log = "logs/payload_normal.jsonl"
    ...
    def _detection_loop(self):
        ...
        while self._running:
            ...
            payload_result = self._classify_payload(pkt)
            payload_label = payload_result.get("payload_label", "NO_PAYLOAD")
            payload_category = payload_result.get("category", "No Payload")
            ...
            self._log_payload_classification(pkt, payload_result)

            # Run signature detection
            alerts = self.detection_engine.process_packet(pkt)

            # Run payload signature detection and categorization
            payload_alerts = self._detect_payload_alerts(pkt, payload_result)
            if payload_alerts:
    ...
    def _classify_payload(self, pkt: dict) -> dict:
        """Classify payload as ATTACK/NORMAL/NO_PAYLOAD."""
        if not self.payload_detector:
            return {
                "status": "detector_disabled",
                "payload_label": "NO_PAYLOAD",
                "category": "Detector Disabled",
                "is_threat": False,
                "detections": [],
            }

        payload = pkt.get("payload")
        rule_name = self._infer_rule_name(pkt)
        return self.payload_detector.detect(pkt, rule_name=rule_name, payload=payload)

    def _detect_payload_alerts(self, pkt: dict, result: dict) -> list:
        """Run payload signature detector and convert findings into alert dicts."""
        ...
        rule_name = result.get("rule_name") or self._infer_rule_name(pkt)
        det = detections[0]
        details = det.get("details", {})
        return [{
            "type": "Payload Attack",
            "rule_name": f"Payload Signature: {rule_name}",
            "protocol": pkt.get("protocol", "OTHER"),
            "severity": det.get("severity", "HIGH"),
            "source": pkt.get("src", "Unknown"),
            "target": pkt.get("dst", "Unknown"),
            "dst_port": pkt.get("dst_port"),
            "packet_count": 1,
            "window": 1,
            "threshold": 1,
            "timestamp": pkt.get("timestamp"),
            "category": det.get("category", "Payload Threat"),
            "matched_signatures": details.get("matched_signatures", []),
            "confidence": details.get("total_score", 0.0),
        }]

    def _log_payload_classification(self, pkt: dict, result: dict):
        """Persist payload ATTACK/NORMAL classifications into separate logs."""
        payload = pkt.get("payload")
        if not payload:
            return
        ...
        event = {
            ...
            "payload_label": result.get("payload_label", "NO_PAYLOAD"),
            "category": result.get("category", "Unknown"),
            "is_threat": result.get("is_threat", False),
            "detections": [d.get("category", "Payload Threat") for d in result.get("detections", [])],
        }

        file_path = self._payload_attack_log if event["is_threat"] else self._payload_normal_log
        try:
            os.makedirs("logs", exist_ok=True)
            ...
            logger.debug(f"Payload classification log write failed: {e}")

    def _infer_rule_name(self, pkt: dict) -> str:
        """Best-effort rule naming for payload detections."""
        for rule in self.detection_engine.rules:
            try:
                if self.detection_engine._match_rule(pkt, rule):
                    return rule.get("name", "Generic Payload Rule")
            except Exception:
                continue
        proto = pkt.get("protocol", "UNKNOWN")
        port = pkt.get("dst_port")
        if port:
            return f"{proto} payload on port {port}"
        return f"{proto} payload"
...
def main():
    ...
    logger.info("Detection Engine initialized with signature-based threshold tuning")
    if hasattr(detection_engine, 'get_tuner_stats'):
        tuner_stats = detection_engine.get_tuner_stats()
        logger.info(
            f"  Tuner algorithm: {tuner_stats.get('algorithm', 'unknown')} | "
            f"signatures={tuner_stats.get('signature_count', 0)}"
        )
    ...
    controller = CaptureController(
        ...
        gui_queue        = gui_queue,
        payload_detector = payload_detector,
